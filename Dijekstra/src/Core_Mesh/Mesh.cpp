...
{	
	if (!boundaryLabeled) 
		...
		return;//only need to label boundaries once
	...
	{		
		...
		//else 
		//{	//The following edge order is not important.		
...
{	
	...
	{	
		...
		if( !strcmp(str, "Vertex" ) ) 
		{
			...
			v->id()    = id;
			std::vector<Halfedge *> heList; //storing the neighboring halfedges, for efficient edge searching in initialization
			ver2TraitIndex[v] = v_adjHeList.size();
...
void Mesh::copyTo( Mesh & tMesh )
{
		...
		viter!=m_verts.end(); ++viter)
	{
		...
		//tMesh.v_adjHeList.push_back(heList);
		//tMesh.ver2TraitIndex[v] = i++;
	}
	tMesh.maxVid = maxVid;
	...
	for(;fiter!=m_faces.end(); ++fiter)
	{
		...
		Halfedge * he[3];
		Halfedge * nhe[3];
		he[0] = f->he();		he[1] = he[0]->he_next();		he[2] = he[1]->he_next();
		for (int j=0; j<3; ++j)
		{
			nhe[j] = new Halfedge();
			Vertex * v1 = he[j]->vertex();
		...
		for (int j=0;j<3;++j)
		{
			nhe[j]->he_next()=nhe[(j+1)%3];
			nhe[j]->he_prev()=nhe[(j+2)%3];
		}
		nf->he()=nhe[0];
	...
	for(fiter = m_faces.begin();fiter!=m_faces.end(); ++fiter)
	{
		...
		Halfedge * nhe[3];
		he[0] = f->he();		he[1] = he[0]->he_next();		he[2] = he[1]->he_next();
		nhe[0] = nf->he();		nhe[1] = nhe[0]->he_next();		nhe[2] = nhe[1]->he_next();
		for (int i=0;i<3;++i)
		{
			Edge * e = he[i]->edge();
			if (he[i]==e->he(0))
			{
				...
				for (int j=0;j<3;++j)
				{
					...
					twin_he = twin_he->he_next();
					twin_nhe = twin_nhe->he_next();
				}
...
{
	Halfedge * he0 = v0->most_clw_in_halfedge();
	if(he0->source() == v1)
		return he0->edge();
	Halfedge * he=he0->ccw_rotate_about_target();
	while (he!=he0 && he)
	{
		if (he->source()==v1)
			return he->edge();
	...
	if (!v0->boundary())
		return NULL;  //in this case, it is not an edge
	he = v0->most_clw_out_halfedge();
	if (he->target()==v1)
		return he->edge();
	else
		return NULL;
...
Halfedge * Mesh::idHalfedge( int id0, int id1 )
{
	Edge * e = idEdge( id0, id1 );
	if (!e)
		return NULL;
	Halfedge * he = e->he(0);
...
{	
	...
	for(i = 0; i < 3; i ++ )
	{
		...
		hes[i]->vertex() = ver[i];
		ver[i]->he() = hes[i];
		std::vector<Halfedge *> & adjHeList = v_adjHeList[ver2TraitIndex[ver[i]]];
	...
	for( i = 0; i < 3; i ++ )
	{
		...
		Edge * e = NULL;
		std::vector<Halfedge *> & adjHeList = v_adjHeList[ver2TraitIndex[ev1]];
